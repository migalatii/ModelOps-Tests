#!/usr/bin/env python3
"""
evaluate_metric_based.py

Computation-Based Evaluation Script (exact_match, rouge_l, bleu).
Evaluates deployed Weather Agent against dataset using deterministic metric calculations.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Computation-Based Vertex AI Metric Evaluation")
    ...
    parser.add_argument("--metrics", type=str, default="exact_match,rouge_l", help="Computation metrics list")
...
def load_dataset(dataset_path: str, allow_mock: bool) -> pd.DataFrame:
    print(f"[INFO] Ingesting computation evaluation dataset: {dataset_path}")
    try:
...
def main():
    args = parse_args()
    print(f"[INFO] Running Computation-Based Evaluation for Agent: {args.agent_id}")
    ...
    eval_task = EvalTask(
        dataset=eval_df,
        metrics=metrics,
        experiment=f"metric-based-eval-{args.agent_id.split('/')[-1]}"
    )
    ...
    print("\n==========================================")
    print("   COMPUTATION-BASED EVALUATION METRICS   ")
    print("==========================================")
